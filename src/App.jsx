...
const SISTEMAS_OPTS = [
  "Sistema POS","Sistema RRHH","Sistema de Reportes","Facturación Electrónica",
  "Sitio Web","App Móvil","KDS (Kitchen Display)","Sistema de Depósitos","Otro"
];
...
export default function App() {
  ...
  const addRec = async () => {
    ...
    await post("dapos_recurrentes", {
      no_contrato: form.no_contrato || null, cliente: form.cliente,
      servicio: form.servicio || "Otro", monto: parseFloat(form.monto) || 0,
      tasa: svc.rate, fecha_inicio: form.fecha_inicio || TODAY, es_nuevo: !!form.es_nuevo
    });
    await load(); setSaving(false); closeModal();
  };
  const toggleNuevo = async (r) => {
    await patch("dapos_recurrentes", r.id, { es_nuevo: !r.es_nuevo });
    await load();
  };
  const toggleEstadoRec = async (r) => {
    await patch("dapos_recurrentes", r.id, { estado: r.estado === "Activo" ? "Cancelado" : "Activo" });
    await load();
  };
  ...
  const addOne = async () => {
    setSaving(true);
    await post("dapos_unicos", {
      no_factura: form.no_factura || null, cliente: form.cliente,
      descripcion: form.descripcion, monto: parseFloat(form.monto) || 0,
      fecha: form.fecha || TODAY, estado: "Pendiente"
    });
    await load(); setSaving(false); closeModal();
  };
  ...
  const addSis = async () => {
    ...
    await post("sistemas_contratos", {
      cliente: form.cliente, telefono: form.telefono || null, sistema: form.sistema,
      monto_total: t, cuota_mensual: form.cuota_mensual ? parseFloat(form.cuota_mensual) : Math.round(t / m),
      total_cuotas: m, fecha_inicio: form.fecha_inicio || TODAY, notas: form.notas || null
    });
    await load(); setSaving(false); closeModal();
  };
  const delSis = async (id) => { await del("sistemas_contratos", id); setPays(p => p.filter(x => x.contrato_id !== id)); await load(); };

  const addPay = async () => {
    setSaving(true);
    const c = sis.find(s => s.id === selSis);
    await post("sistemas_pagos", {
      contrato_id: selSis, no_cuota: sisPaidCt(selSis) + 1,
      monto: parseFloat(form.monto) || c.cuota_mensual,
      fecha_pago: form.fecha || TODAY, estado: "Pagado"
    });
    await load(); setSaving(false); closeModal();
  };
  ...
  // ── UI Atoms ──────────────────────────────────────────────────────────────────
  const Bdg = ({ s }) => {
    const m = { Activo: "bg-emerald-100 text-emerald-700", Cancelado: "bg-red-100 text-red-600",
      Pagado: "bg-sky-100 text-sky-700", Pendiente: "bg-amber-100 text-amber-700",
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex gap-2 px-5 py-4 border-t">
          ...
          <button onClick={onOk} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen bg-slate-50 flex" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <aside className="w-52 bg-[#0a1628] flex flex-col py-5 px-3 flex-shrink-0 min-h-screen">
        ...
        <nav className="space-y-0.5 flex-1">
          {[{ k: "dashboard", l: "Dashboard", I: TrendingUp },
            { k: "dapos",     l: "D-APOS",    I: Building2  },
            { k: "sistemas",  l: "Mis Sistemas", I: Cpu }].map(({ k, l, I }) => (
        ...
        <div className="px-2 pt-4 border-t border-white/5">
          <button onClick={load} className="flex items-center gap-2 text-slate-600 text-xs hover:text-slate-400 transition-colors">
            ...
            <div className="space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl p-4 shadow-sm border border-slate-100 cursor-pointer hover:border-violet-300 hover:shadow-md transition-all group" onClick={() => nav("sistemas","contratos")}>
                  <div className="flex justify-between items-center">
                    <div><div className="font-semibold text-slate-700 text-sm mb-1">Sistemas en Cuotas Mensuales</div><div className="text-2xl font-bold text-violet-600">{sis.length}</div><div className="text-xs text-slate-400 mt-1">contratos registrados</div></div>
              ...
              {sub === "recurrentes" && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="flex justify-between items-center px-5 py-4 border-b border-slate-50">
                    ...
                    <button onClick={() => { setForm({}); setModal("addR"); }} className="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-emerald-700 transition-colors"><PlusCircle size={15} /> Agregar</button>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {rec.map(r => {
                          ...
                          return (
                            <tr key={r.id} className={`border-b border-slate-50 hover:bg-slate-50/80 transition-colors ${r.estado==="Cancelado"?"opacity-40":""}`}>
                              ...
                              <td className="py-3 px-4 text-right text-slate-700 tabular-nums">{fCRC(r.monto)}</td>
                              <td className="py-3 px-4 text-right font-bold text-emerald-600 tabular-nums">{fCRC(comm)}</td>
                              <td className="py-3 px-4 text-center">
                                <button onClick={() => toggleNuevo(r)} title="Servicio nuevo → ₡10.000 fijo" className={`w-7 h-7 rounded-lg flex items-center justify-center mx-auto transition-all ${r.es_nuevo?"bg-amber-400 shadow-sm":"bg-slate-100 hover:bg-amber-100"}`}>
                              ...
                              <td className="py-3 px-4"><button onClick={() => delRec(r.id)} className="text-slate-200 hover:text-red-400 transition-colors p-1"><Trash2 size={13} /></button></td>
                            </tr>
                          );
                        })}
                      </tbody>
                  ...
                  <div className="px-5 py-3 flex items-center gap-2 text-xs text-slate-400 border-t border-slate-50 bg-slate-50/50">
                    ...
                    Activa la estrella <span className="text-amber-600 font-semibold">⭐ en servicios nuevos</span> para aplicar comisión fija de ₡10,000 · Al mes siguiente desactivala y vuelve al %
                  </div>
                </div>
              )}
              ...
              {sub === "unicos" && (
                <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                  <div className="flex justify-between items-center px-5 py-4 border-b border-slate-50">
                    ...
                    <button onClick={() => { setForm({ fecha: TODAY }); setModal("addO"); }} className="flex items-center gap-2 bg-sky-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-sky-700 transition-colors"><PlusCircle size={15} /> Agregar</button>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {one.map(o => (
                          <tr key={o.id} className="border-b border-slate-50 hover:bg-slate-50/80">
                            ...
                            <td className="py-3 px-4 font-medium text-slate-800">{o.cliente}</td>
                            ...
                            <td className="py-3 px-4"><button onClick={() => delOne(o.id)} className="text-slate-200 hover:text-red-400 transition-colors p-1"><Trash2 size={13} /></button></td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
              ...
              {sub === "contratos" && (
                <div className="space-y-4">
                  <div className="flex justify-end">
                    <button onClick={() => { setForm({}); setModal("addS"); }} className="flex items-center gap-2 bg-violet-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-violet-700 transition-colors"><PlusCircle size={15} /> Nuevo Contrato</button>
                  </div>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {sis.map(s => {
                      ...
                      return (
                        <div key={s.id} className="bg-white rounded-xl p-5 shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <div className="flex flex-col items-end gap-1.5">
                              <Bdg s={s.estado} />
                              <button onClick={() => delSis(s.id)} className="text-slate-200 hover:text-red-400 p-0.5"><Trash2 size={12} /></button>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-2 mt-4">
                            <button onClick={() => { setSelSis(s.id); setModal("viewP"); }} className="flex-1 py-2 text-xs rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 flex items-center justify-center gap-1.5 transition-colors"><FileText size={12} /> Ver Pagos</button>
                            <button onClick={() => { setSelSis(s.id); setForm({ monto: s.cuota_mensual, fecha: TODAY }); setModal("addP"); }} className="flex-1 py-2 text-xs rounded-lg bg-violet-600 text-white hover:bg-violet-700 flex items-center justify-center gap-1.5 transition-colors font-semibold"><CheckCircle size={12} /> Registrar Pago</button>
                          </div>
                          {s.notas && <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-50 leading-relaxed">📝 {s.notas}</div>}
                        </div>
                      );
                    })}
                  </div>
                              ...
                              <tbody>
                                {ps.map(p => (
                                  <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50">
                                    <td className="py-2.5 px-4 text-slate-600 font-medium">Cuota #{p.no_cuota}</td>
      ...
      {modal === "addR" && (
        <Mdl title="Agregar Contrato Recurrente D-APOS" onOk={addRec}>
          ...
          <F lbl="Cliente (Nombre completo)"><input className={inp} placeholder="Nombre del cliente" value={form.cliente||""} onChange={e=>setForm({...form,cliente:e.target.value})} /></F>
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 p-4 rounded-xl">
            ...
            <label htmlFor="cn" className="text-sm text-amber-800 cursor-pointer leading-snug">
              <div className="font-bold">¿Nueva instalación este mes?</div>
              <div className="text-xs text-amber-600 mt-0.5">Se aplicará comisión fija de <strong>₡10,000</strong> en lugar del porcentaje (solo el primer mes)</div>
            </label>
          </div>
          {form.servicio && form.monto && (
            <div className="bg-emerald-50 rounded-xl p-3.5 flex justify-between items-center">
              <span className="text-emerald-700 text-sm font-medium">Comisión estimada:</span>
      ...
      {modal === "addO" && (
        <Mdl title="Agregar Venta / Pago Único" onOk={addOne}>
          ...
          <F lbl="Descripción"><input className={inp} placeholder="ej: Venta de equipo, Instalación cámaras..." value={form.descripcion||""} onChange={e=>setForm({...form,descripcion:e.target.value})} /></F>
          <F lbl="Monto de la Venta (₡)"><input className={inp} type="number" placeholder="ej: 250000" value={form.monto||""} onChange={e=>setForm({...form,monto:e.target.value})} /></F>
          <F lbl="Fecha de Factura"><input className={inp} type="date" value={form.fecha||TODAY} onChange={e=>setForm({...form,fecha:e.target.value})} /></F>
          {form.monto && (
            <div className="bg-sky-50 rounded-xl p-3.5 flex justify-between items-center">
              <span className="text-sky-700 text-sm font-medium">Comisión (1%):</span>
      ...
      {modal === "addS" && (
        <Mdl title="Nuevo Contrato de Sistema" onOk={addSis}>
          ...
          <F lbl="Sistema / Producto">
            <select className={inp} value={form.sistema||""} onChange={e=>setForm({...form,sistema:e.target.value})}>
              <option value="">Seleccionar...</option>
              {SISTEMAS_OPTS.map(s=><option key={s} value={s}>{s}</option>)}
            </select>
          </F>
          ...
          <F lbl="Notas"><input className={inp} placeholder="Detalle del contrato..." value={form.notas||""} onChange={e=>setForm({...form,notas:e.target.value})} /></F>
          {form.monto_total && form.total_cuotas && (
            <div className="bg-violet-50 rounded-xl p-3.5 flex justify-between items-center">
              <span className="text-violet-700 text-sm font-medium">Cuota mensual calculada:</span>
        ...
        return (
          <Mdl title="Registrar Pago de Cuota" onOk={addPay}>
            <div className="bg-violet-50 rounded-xl p-4">
              <div className="font-bold text-violet-800">{c?.cliente}</div>
              <div className="text-sm text-violet-600 mt-0.5">{c?.sistema} · Cuota <strong>#{sisPaidCt(selSis)+1}</strong> de {c?.total_cuotas}</div>
              <div className="text-xs text-violet-400 mt-1">Pendiente: {fCRC((parseFloat(c?.monto_total)||0) - sisPaid(selSis))}</div>
            </div>
        ...
        return (
          <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
              <div className="flex justify-between items-center px-5 py-4 border-b">
                <div><div className="font-bold text-slate-800">{c?.cliente}</div><div className="text-sm text-violet-600 font-semibold">{c?.sistema}</div></div>
                ...
                {ps.length === 0
                  ...
                  : ps.map(p => (
                    <div key={p.id} className="flex justify-between items-center py-3 border-b border-slate-50 last:border-0">
                      <div><div className="text-sm font-semibold text-slate-700">Cuota #{p.no_cuota}</div><div className="text-xs text-slate-400">{p.fecha_pago}</div></div>
                      <div className="flex items-center gap-2"><div className="font-bold text-sm tabular-nums">{fCRC(p.monto)}</div><Bdg s={p.estado} /></div>
                    </div>
                  ))}
              </div>
